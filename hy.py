...
def get_real_certificate(base_dir, domain, email="hina@example.com"):
...
def create_config(base_dir, port, password, cert_path, key_path, domain, enable_web_masquerade=True,
                  custom_web_dir=None, enable_port_hopping=False, obfs_password=None, enable_http3_masquerade=False):
    ...
    try:
        ...
        try:
            if os.path.exists(f"{nginx_web_dir}/index.html"):
                subprocess.run(['sudo', 'cp', f'{nginx_web_dir}/index.html', f'{nginx_web_dir}/index.html.backup'],
                               check=True)
        ...
        if os.path.exists(web_dir):
            ...
            try:
                subprocess.run(['sudo', 'find', web_dir, '-type', 'f', '-exec', 'cp', '{}', nginx_web_dir, ';'],
                               check=True)
...
def show_client_setup(config_link, server_address, port, password, use_real_cert, enable_port_hopping=False,
                      obfs_password=None, enable_http3_masquerade=False):
    """显示客户端连接指南"""
    # 构建端口范围
    port_range = None
    if enable_port_hopping:
...
""")
        ...
        show_client_setup(config_link, server_address, port, password, use_real_cert, args.port_hopping,
                          args.obfs_password, args.http3_masquerade)
...
""")
        ...
        if args.simple:
            ...
            port = args.port if args.port else 555
            password = args.password if args.password else "hina0219@"
            ...
            result = deploy_hysteria2_complete(
                ...
                domain=args.domain,
                email=args.email if args.email else "hina@example.com",
        ...
        # 防墙优化配置
        port = args.port if args.port else 555  # 默认使用443端口
        password = args.password if args.password else "hina0219@"
        domain = args.domain
        email = args.email if args.email else "hina@example.com"
        ...
        print(
            f"🛡️ 防护级别: {'顶级防护' if args.port_hopping and args.obfs_password and args.http3_masquerade else '高级防护' if (args.port_hopping and args.obfs_password) or (args.obfs_password and args.http3_masquerade) else '中级防护' if args.port_hopping or args.obfs_password or args.http3_masquerade else '基础防护'}")
        ...
        config_path = create_config(base_dir, port, password, cert_path, key_path,
                                    server_address, args.web_masquerade, web_dir, args.port_hopping, args.obfs_password,
                                    args.http3_masquerade)
...
def setup_port_hopping_iptables(port_start, port_end, listen_port):
    ...
    try:
        ...
        # 清理可能存在的旧规则
        try:
            subprocess.run(['sudo', 'iptables', '-t', 'nat', '-D', 'PREROUTING', '-p', 'udp', '--dport',
                            f'{port_start}:{port_end}', '-j', 'DNAT', '--to-destination', f':{listen_port}'],
                           check=False, capture_output=True)
        except:
            pass
...
def deploy_hysteria2_complete(server_address, port=555, password="hina0129@", enable_real_cert=False, domain=None,
                              email="hina@example.com", port_range=None, enable_bbr=False):
    """
...
"""
        ...
        setup_config_download_service(server_address, v2rayn_file, clash_file, hysteria_official_file,
                                      hysteria_client_hopping_file, subscription_file, subscription_plain_file,
                                      config_file)
...
def setup_config_download_service(server_address, v2rayn_file, clash_file, hysteria_official_file,
                                  hysteria_client_hopping_file, subscription_file, subscription_plain_file, json_file):
    """设置配置文件下载服务 - 完全自动化"""
    try:
        print("🌐 设置配置文件下载服务...")
...
def show_final_summary(server_address, port, port_range, password, obfs_password, config_link,
                       enable_port_hopping=False, download_links=None, num_ports=None):
    import urllib.parse
    ...
    print("\033[32m" + "║" + " " * 78 + "║" + "\033[0m")
    print("\033[32m" + "║" + "✅ 已创建全局管理命令，输入 'ysy' 进入管理菜单".center(74) + "║" + "\033[0m")
    print("\033[32m" + "║" + " " * 78 + "║" + "\033[0m")
    print("\033[32m" + "║" + "💡 菜单功能：1-查看节点 2-查看配置 3-服务状态 4-重启服务 5-查看日志 6-删除服务".center(
        66) + "║" + "\033[0m")
    print("\033[32m" + "║" + " " * 78 + "║" + "\033[0m")
...
'''
        ...
        # 创建kk命令文件
        kk_script_path = "/usr/local/bin/ysy"
        try:
...
def generate_multi_port_subscription(server_address, password, obfs_password, port_start, port_end, base_dir,
                                     num_configs=100):
    """
    生成多端口v2rayN订阅文件
